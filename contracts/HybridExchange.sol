...
contract HybridExchange is LibOrder, LibMath, LibRelayer, LibExchangeErrors {
    ...
    function matchOrders(
        ...
        uint256[] memory filledBaseAmounts,
        OrderAddressSet memory orderAddressSet
    ) public {
        require(canMatchOrdersFrom(orderAddressSet.relayer), INVALID_SENDER);

        uint256 takerFeeRate = getAsTakerFeeRateFromOrderData(takerOrderParam.data);
        ...
        returns (OrderInfo memory orderInfo)
    {
        ...
        orderInfo.orderHash = hashOrder(order);
    ...
    {
        result.baseTokenFilledAmount = filledBaseAmount;
        result.quoteTokenFilledAmount = convertBaseToQuote(makerOrderParam, filledBaseAmount);
        ...
        if (rebateRate > makerRawFeeRate) {
            ...
            uint256 makerRebateRate = min(
                rebateRate.sub(makerRawFeeRate),
                takerFeeRate
            );
            ...
            uint256 makerFeeRate = makerRawFeeRate.sub(rebateRate);
