...
contract('Match', async accounts => {
    ...
    const getOrderSignature = async (order, exchange, baseToken, quoteToken) => {
        ...
        const isValid = await isValidSignature(
            order.trader,
            toRpcSig(signature.v, signature.r, signature.s),
            orderHash
        );
    ...
    const matchTest = async config => {
        ...
        const res = await exchange.methods
            .matchOrders(takerOrder, makerOrders, filledBaseTokenAmounts, {
        ...
        for (let i = 0; i < Object.keys(assertDiffs).length; i++) {
            ...
            for (let j = 0; j < Object.keys(assertDiffs[tokenSymbol]).length; j++) {
                ...
                assertEqual(actualDiff, expectedDiff, allowPrecisionError, `${balanceKey}`);
            }
        }
    ...
    it('taker sell(limit & market), taker full match', async () => {
        ...
        const testConfig = {
            ...
            filledBaseTokenAmounts: [toWei('10'), toWei('10')],
            assertDiffs: {
                TT: {
                    u1: toWei('-20'),
                    u2: toWei('10'),
    ...
    it('taker sell(limit & market), maker full match', async () => {
        ...
        await limitAndMarketTestMatch({
            filledBaseTokenAmounts: [toWei('1952')],
            users: { u1, u2, u3, relayer },
            baseTokenConfig: {
                name: 'TestToken',
                symbol: 'TT',
    ...
    it('taker buy(limit), taker full match', async () => {
        await matchTest({
            filledBaseTokenAmounts: [toWei('1952'), toWei('6472.22')],
            assertFilled: {
    ...
    it('taker buy(market), taker full match', async () => {
        await matchTest({
            ...
            filledBaseTokenAmounts: [toWei('1952'), toWei('6525.004396825396825396')],
            assertDiffs: {
                TT: {
    ...
    // ║         │  TT      │ WETH          │                                               ║
    // ╠═════════╪══════════╪═══════════════╪═══════════════════════════════════════════════╣
    // ║ u1      │  1952    │ -75.5683216   │ -(0.036821 * 1952) * (1 + 0.05) - 0.1         ║
    // ╟─────────┼──────────┼───────────────┼───────────────────────────────────────────────╢
    // ║ u2      │ -1952    │ 71.05584608   │ 0.036821 * 1952 * 0.99 - 0.1                  ║
    // ╟─────────┼──────────┼───────────────┼───────────────────────────────────────────────╢
    // ║ relayer │ 0        │  4.51247552   │ (0.036821 * 1952) * (0.05 + 0.01) + 0.2       ║
    // ╚═════════╧══════════╧═══════════════╧═══════════════════════════════════════════════╝
    ...
    // ╟─────────┼──────────┼───────────────┼───────────────────────────────────────────────────╢
    // ║ u2      │   102    │ -0.085238446  │ -0.0008273 * 102 * (1 + 0.01) - 0.00001     ║
    // ╟─────────┼──────────┼───────────────┼───────────────────────────────────────────────────╢
    // ║ relayer │    0     │ 0.005083076   │ (0.0008273 * 102) * (0.01 + 0.05) + 0.00002 ║
    // ╚═════════╧══════════╧═══════════════╧═══════════════════════════════════════════════════╝
    it('eth, taker market order sell, taker full match, maker fee discount', async () => {
        await setHotAmount(hot, u2, toWei(10000));
        await matchTest({
            filledBaseTokenAmounts: [toWei('102')],
            users: { u1, u2, relayer },
            baseTokenConfig: {
                name: 'TestToken',
                symbol: 'TT',
            ...
            assertDiffs: {
                ...
                ETH: {
                    u1: toWei('0.08015537'),
                    u2: toWei('-0.085238446'),
                    relayer: toWei('0.005083076')
                }
            }
        });
    });
    ...
    it('Invalid taker order will revert', async () => {
        const testConfig = {
            users: { u1, u2, u3, relayer },
            filledBaseTokenAmounts: [toWei('1')],
            baseTokenConfig: {
                name: 'TestToken',
                symbol: 'TT',
                decimals: 18,
